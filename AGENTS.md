...
- **Runtime:** Bun matches the `"bun"` condition and imports `src/index.ts` directly (Bun transpiles TS natively). That's why `src/**/*.ts` is in `files` and the `#*` map resolves at the consumer (`./src/*` is shipped). No `default`/`import` fallback — non-Bun runtimes are intentionally unsupported. A consumer resolving us through Node's algorithm therefore fails with `ERR_PACKAGE_PATH_NOT_EXPORTED`; `@better-auth/cli` does, since its bin has a Node shebang — hence the README's `bun run --bun`.
